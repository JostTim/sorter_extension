...
function handle_request(request, sender, sendResponse) {
  ...
  if (request.action === 'get_browser_infos') {
    get_browser_infos()
      .then((browserInfos) =>
        sendResponse({ result: 'success', data: browserInfos }),
      )
      .catch((error) =>
        sendResponse({ result: 'error', message: error.message }),
      );
  ...
  if (request.action === 'get_associations') {
    get_associations(request.tabs, request.groups, request.options)
      .then((associationsInfos) =>
        sendResponse({ result: 'success', data: associationsInfos }),
      )
      .catch((error) =>
        sendResponse({ result: 'error', message: error.message }),
      );
  ...
  if (request.action === 'saveStorageSync') {
    ensureStorageInitialization()
      .then(() =>
        sendResponse({ result: 'success', message: 'Saved Successfully' }),
      )
      .catch((error) =>
        sendResponse({ result: 'error', message: error.message }),
      );
...
async function get_associations_metadata(associations) {
  ...
  for (let tabId in associations) {
    if (!associations[tabId].g) {
      continue;
    }
    let matching_groups = await browser.tabGroups.query({
      title: associations[tabId].g,
    });
...
async function mapTabsToGroups(tabs, groups, options) {
  ...
  let do_filter = (
    await browser.storage.sync.get('filter_grouped_tabs_from_prompt')
  ).filter_grouped_tabs_from_prompt;
  ...
  const data = {
    tabs: tabs.map((tab) => ({
      ...
      groupName:
        tab.groupId === -1
          ? ''
          : (groups.find((group) => group.id === tab.groupId) || {}).title,
    })),
...
async function prompt_ai_provider(tabs_mapping, options) {
  const prompt =
    'Please make associations of tabs to groups, leaving already existing group / tabs \
  ...
  console.log(
    `Recieved AI api response of ${result.output[0].content[0].text.length} characters`,
  );
...
async function order_tabs(new_associations, options) {
  for (let tabId in new_associations) {
    ...
    if (tab.groupId !== -1) {
      ...
      console.log(
        `Tab with id ${tab.id} is already having a group : ${tab.groupId}`,
      );
      continue;
    }
    ...
    if (matching_groups.length === 1) {
      ...
      if (tab.title !== tabTitle) {
        console.warn(
          `The title of the tab returned by the ai ${tabTitle} doesn't match the one returned by the browser.api ${tab.title}`,
        );
        continue;
      }
      ...
      let sameGroupTabs = await find_all_tabs_for_group(
        new_associations,
        groupName,
      );
      ...
      if (sameGroupTabs.length >= 2) {
        console.log(
          'Would make ',
          sameGroupTabs.length,
          'tabs part of a new group',
          groupName,
        );
      ...
      console.error(
        `Cannot know what to do, several groups with the name ${groupName} exists`,
      );
    }
  }
  return 'success';
...
class AIPrompts {
  static async setup() {
    if ((await AIPrompts.ai_prompts_ids()) === undefined) {
      const prompt =
        'Please make associations of tabs to groups, leaving already existing group / tabs \
            associations, only making tabs with empty group association, linked to existing, \
            or new groups if you find that no exising group seem suitable. \
            ...
            Example : { "1565416846" : { "t" : "Youtube", "g" : "Divertissement" } } \
            Do not write otherwise any text (the output will be parsed by a program)';
      await Storage.set_value('ai_prompts_ids', []);
      AIPrompts.new_prompt(prompt);
    }
  }

  static async new_prompt(prompt) {
    let ai_prompts_ids = await AIPrompts.get_prompts_id('');
    let new_ai_prompt_id = ai_prompts_ids ? ai_prompts_ids[-1] + 1 : 0; // new id = last prompt id + 1
    ai_prompts_ids.push(new_ai_prompt_id);
  }

  static async get_prompts_id() {
    return await Storage.get_value('ai_prompts_ids');
  }

  static async get_uses(id) {
    return await Storage.get_value(`ai_uses_${id}`);
  }

  static async set_uses(id, uses_dict) {
    return await Storage.set_value(`ai_uses_${id}`, uses_dict);
  }

  static async get_prompt(id) {
    return await Storage.get_value(`ai_prompt_${id}`);
  }

  static async set_prompt(id, prompt) {
    return await Storage.set_value(`ai_prompt_${id}`, prompt);
  }
}
